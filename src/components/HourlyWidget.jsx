...
export default function HourlyWidget(data) {
    ...
    return (
        <div className={`${dark ? 'dark-card' : 'light-card'} widget shadow-md h-52 w-40 rounded-lg p-4 m-2 flex flex-col items-center sm:justify-between`}>
            ...
            <div className="temperature flex items-center mt-2 sm:mt-0">
                <div className="icon mr-2">
                    <img src={`${url}/${icon}.png`} alt={summary} className="w-12 h-12" draggable={false} />
                </div>
                <div className="temp text-2xl font-bold">{Math.round(temperature)} °C</div>
            </div>
            <div className="precipitation text-sm text-gray-500 mt-2 sm:mt-0">
                {Math.round(precipitation.total)} %
            </div>
            <div className="wind flex items-center mt-2 sm:mt-0">
                <div className="wind-speed text-sm">{Math.round(wind.speed)} m/s</div>
